...
- Current base: `origin/main` at `3d8911842aa8c7cbf7ee74d02fce91f4b2893be3` after PR #67 merged.
- Working branch intent: implement Task 5 from `docs/roadmaps/2026-06-02-next-five-tasks-roadmap.md` by adding read-only GitHub evidence sync and feedback candidates from saved PR/check/review-thread evidence.
- Recent merged PRs: PR #56 implemented the read-only `audit-replay` CLI path; PR #57 updated this handoff after PR #56 merged; PR #58 merged the command-policy and active-stop control slice; PR #59 hardened command-policy review findings; PR #60 added the dry-run Git/PR planning design; PR #61 implemented dry-run-only `git-pr-plan`; PR #62 added the next-five-tasks roadmap; PR #63 refreshed this handoff and seeded the active business-memory backlog; PR #64 added and hardened the controlled executor fixture; PR #66 wired local executor closeout and next-decision logic; PR #67 merged local `branch_policy` for loop policy, task packets, and dry-run Git/PR planning.
- Current branch scope: `codex/task-5-github-evidence-sync` adds read-only GitHub evidence sync, failing-check candidates, and review-thread readiness blockers. It must not add real executor invocation, GitHub writes, branch creation, commits, pushes, pull requests, merges, releases, or package-publication behavior.
...
- Candidate discovery can ingest saved PR JSON through `--pr-json-file` and convert failing checks into `pr_check_failure` execution candidates.
- `pr-readiness --review-threads-file` can block unresolved actionable current review comments from saved GraphQL `reviewThreads` JSON while ignoring resolved, outdated, and non-actionable feedback.
...
- `docs/roadmaps/2026-06-02-next-five-tasks-roadmap.md` is the current public planning artifact for the next bounded slices; Tasks 1, 2, 3, and 4 are merged, Task 5 is current-tree work, and Tasks 6-7 are planned follow-on gates.
...
- `github-evidence-sync` is read-only live evidence capture. It may write local evidence files, but it must not start GitHub writes, create or edit pull requests, create branches, commit, push, merge, release, or publish packages.
...
- Real executor invocation remains blocked until operator-approved branch/PR materialization policy, resume verification, and result evidence gates are stable and covered by tests.
...
Finish Task 5 from `docs/roadmaps/2026-06-02-next-five-tasks-roadmap.md`: review, verify, and merge the read-only GitHub evidence sync and feedback-candidate slice. After Task 5 merges, Task 6 is next: operator-approved Git/PR materialization. Live Git/PR actions, real executor invocation, merge, release, and package publication remain outside this Task 5 scope.
