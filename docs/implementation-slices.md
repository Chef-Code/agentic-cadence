...
Task 47 projected capability at 42% while this stable headline remains 25%.
...
- `controlled-loop-run-summary`, `controlled-loop-outcome-plan`,
  ...
  `controlled-loop-runner-plan` can summarize the saved runner-adjacent chain,
  map the terminal outcome to a bounded next operator action, bind the
  evidence-file manifest plus controlled command stages, verify target-bound
  manifest approval, and produce a dry-run runner plan without appending audit
