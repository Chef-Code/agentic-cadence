...
Current unattended-operation confidence: 25% (deliberately stable headline token; progress-log records Task 47 projected capability at 42%)
...
- A read-only `loop-tick` now stitches snapshot, candidate election, Cadence
  ...
  runner plan, all without appending audit evidence, starting a runner or
