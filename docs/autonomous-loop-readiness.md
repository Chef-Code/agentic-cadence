...
Current unattended-operation confidence: 25% (deliberately stable headline token; progress-log records Task 79 projected capability at 74%)
...
Task 78 narrows the executor launch wording above:
`controlled-loop-runner-executor-invocation` can now consume reviewed runner
executor-invocation readiness plus a runner-bound `executor-invocation-plan.v1`,
...
Task 79 narrows the retry inspection wording above:
`controlled-loop-runner-stage-retry-inspection-preparation` can now consume a
reviewed failed or blocked
`controlled-loop-runner-stage-retry-outcome-plan.v1`, recheck the retry
closeout/execution/source chain and reviewed outcome checksum, and emit a
deterministic read-only inspection-preparation packet. This still does not
perform the inspection, start a second retry, continue the runner or loop,
