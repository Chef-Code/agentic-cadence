...
- A read-only `loop-tick` now stitches snapshot, candidate election, Cadence
  ...
  `controlled-loop-runner-stage-invocation-boundary` prepares exact approved
  argv/cwd/output/timeout, `execution_authority`, and
  `allowed_side_effects_when_executed` boundary evidence without starting a
  process, all without appending audit evidence, starting an executor, retrying
  an executor, continuing the loop,
