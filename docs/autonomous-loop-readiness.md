...
Current unattended-operation confidence: 25% (deliberately stable headline token; progress-log records Task 41 projected capability at 36%)
...
- A read-only `loop-tick` now stitches snapshot, candidate election, Cadence
  ...
  that controlled plan before closeout, all without starting a runner,
  retrying an executor, continuing the loop, starting a PR action, writing
  GitHub state, or merging.
