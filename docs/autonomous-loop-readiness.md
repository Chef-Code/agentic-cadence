...
Current unattended-operation confidence: 25% (deliberately stable headline token; progress-log records Task 68 projected capability at 63%)
...
- `controlled-loop-runner-stage-execute` can now consume that continuation
  ...
  declared `epoch_started` and `execution_start_decision` effects, append one
  stage-execution audit record after process start, and stop without retrying,
  executing a second stage, continuing the loop, invoking an executor, or
