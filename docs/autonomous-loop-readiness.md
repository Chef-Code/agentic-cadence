...
Current unattended-operation confidence: 25% (deliberately stable headline token; progress-log records Task 75 projected capability at 70%)
...
Task 74 narrows the retry wording above:
...
execution evidence and map it to a post-retry operator target. They remain
read-only and still cannot start a second retry, select a next stage, complete
the runner, continue the runner or loop, append audit evidence, invoke
executors, or write Git/GitHub state.
