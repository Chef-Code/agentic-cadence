...
selected stage and upstream runner chain before preparing either initial or
continuation-backed deterministic stage-execution approval targets without
executing the stage, pins reviewed stage-input binding checksums for
continuation-backed targets, and
...
stage or invoking an executor, binds the separate executor-task approval for
continuation stage-2 `start-governed-execution`, and
`controlled-loop-runner-stage-invocation-boundary` evidence that re-verifies
the approved selected stage and prepares the exact initial or continuation
argv, working-directory policy, evidence-output policy, timeout policy, and
boundary checksum without starting a process, and
`controlled-loop-runner-stage-execute` evidence that
...
exact executor task file before continuation readiness, while still emitting
no approval token, starting no process or epoch, appending no audit evidence,
...
`controlled-loop-runner-stage-execution-readiness` consumes exactly one stage
selection source: either saved completed `controlled-loop-runner-next-stage.v1`
for the initial stage, or saved completed
`controlled-loop-runner-next-stage-continuation.v1` plus a matching
`controlled-loop-runner-stage-input-binding.v1` packet for continuation
stages. It also consumes `controlled-loop-runner-start.v1`,
`controlled-loop-runner-plan.v1`, and `controlled-loop-runner-dry-run.v1`.
Continuation-backed readiness also requires
`--expected-stage-input-binding-checksum` so the operator-reviewed binding
packet is pinned exactly. It rechecks the selected stage, anchors, checksums,
and dry-run stage sequence, then prepares a stage-execution approval target
...
readiness timestamp, selected stage, selection-source checksum, and upstream
runner checksums, and append no audit evidence. Continuation-backed readiness
also includes `stage_selection_source: continuation` and binds the matching
...
either the initial `controlled-loop-runner-next-stage.v1` packet or the
continuation `controlled-loop-runner-next-stage-continuation.v1` packet plus a
matching `controlled-loop-runner-stage-input-binding.v1` packet,
`controlled-loop-runner-start.v1`, `controlled-loop-runner-plan.v1`, and
`controlled-loop-runner-dry-run.v1` packets. It rechecks the full runner
chain, re-verifies the saved operator approval with the local approval secret,
and confirms the approved selected stage before emitting the exact stage
invocation boundary without starting a process:
...
Example continuation stage-2 boundary:

```bash
agentic-cadence --root examples/first-run/work/runtime controlled-loop-runner-stage-invocation-boundary --controlled-loop-runner-stage-execution-approval-file controlled-loop-runner-stage-execution-approval.json --controlled-loop-runner-stage-execution-readiness-file controlled-loop-runner-stage-execution-readiness.json --controlled-loop-runner-next-stage-continuation-file controlled-loop-runner-next-stage-continuation.json --controlled-loop-runner-stage-input-binding-file controlled-loop-runner-stage-input-binding.json --expected-stage-input-binding-checksum sha256:<reviewed-stage-input-binding-checksum> --controlled-loop-runner-start-file controlled-loop-runner-start.json --controlled-loop-runner-plan-file controlled-loop-runner-plan.json --controlled-loop-runner-dry-run-file controlled-loop-runner-dry-run.json --stage-cwd examples/first-run/work/repo --stage-output-file controlled-loop-runner-stage-output.json --stage-timeout-seconds 300 --expected-operator-id operator@example.test --approval-secret-env CADENCE_OPERATOR_APPROVAL_SECRET --stage-number 2
```
...
For continuation stage-2 `start-governed-execution`, the emitted argv includes
...
matching `controlled-loop-runner-stage-input-binding.v1` packet,
`controlled-loop-runner-start.v1`, `controlled-loop-runner-plan.v1`, and
`controlled-loop-runner-dry-run.v1` packets. It rechecks the full runner chain
and invocation boundary before process start, requires the exact approved
argv/cwd/output/timeout policy, and executes that one stage command with
`shell=False`:
...
Example continuation stage-2 execution:
...
nonempty JSON evidence. The command observes explicit stdout `side_effects`
...
for target-bound operator approval. The packet does not select another stage, emit
...
`controlled-loop-runner-stage-retry-approval` consumes a saved planned
`controlled-loop-runner-stage-retry-plan.v1` packet, the source evidence chain
referenced by that retry plan, and a target-bound `operator-approval.v1` packet
with purpose `controlled_loop_runner_stage_retry`. The approval target checksum
must equal the retry plan's `retry_approval_target_checksum`. Continuation
retry approvals also require the matching continuation and stage-input binding
evidence plus the reviewed stage-input binding checksum. The command emits only
approval evidence and still does not prepare a retry boundary or execute the
retry.

```bash
agentic-cadence --root examples/first-run/work/runtime controlled-loop-runner-stage-retry-approval --controlled-loop-runner-stage-retry-plan-file controlled-loop-runner-stage-retry-plan.json --controlled-loop-runner-stage-outcome-plan-file controlled-loop-runner-stage-outcome-plan.json --controlled-loop-runner-stage-closeout-file controlled-loop-runner-stage-closeout.json --controlled-loop-runner-stage-execution-file controlled-loop-runner-stage-execution.json --controlled-loop-runner-start-file controlled-loop-runner-start.json --controlled-loop-runner-plan-file controlled-loop-runner-plan.json --controlled-loop-runner-dry-run-file controlled-loop-runner-dry-run.json --approval-file operator-approval-controlled-runner-stage-retry.json --expected-operator-id operator@example.test --approval-secret-env CADENCE_OPERATOR_APPROVAL_SECRET --stage-number 1
```

```bash
agentic-cadence --root examples/first-run/work/runtime controlled-loop-runner-stage-retry-approval --controlled-loop-runner-stage-retry-plan-file controlled-loop-runner-stage-retry-plan.json --controlled-loop-runner-stage-outcome-plan-file controlled-loop-runner-stage-outcome-plan.json --controlled-loop-runner-stage-closeout-file controlled-loop-runner-stage-closeout.json --controlled-loop-runner-stage-execution-file controlled-loop-runner-stage-execution.json --controlled-loop-runner-next-stage-continuation-file controlled-loop-runner-next-stage-continuation.json --controlled-loop-runner-stage-input-binding-file controlled-loop-runner-stage-input-binding.json --expected-stage-input-binding-checksum sha256:<reviewed-stage-input-binding-checksum> --controlled-loop-runner-start-file controlled-loop-runner-start.json --controlled-loop-runner-plan-file controlled-loop-runner-plan.json --controlled-loop-runner-dry-run-file controlled-loop-runner-dry-run.json --approval-file operator-approval-controlled-runner-stage-retry.json --expected-operator-id operator@example.test --approval-secret-env CADENCE_OPERATOR_APPROVAL_SECRET --stage-number 2
```

Valid retry-approval packets emit
`controlled-loop-runner-stage-retry-approval.v1`, recheck the retry plan and
all source anchors, verify the operator approval signature, and set
`next_controlled_action` to `prepare_controlled_runner_stage_retry_boundary`.
They do not emit stage-execution readiness, prepare the retry boundary, start a
process, execute a retry, invoke an executor, continue the loop, append audit
evidence, write Git/GitHub state, merge, release, publish packages, assign
roles, or schedule agents.

`controlled-loop-runner-stage-retry-boundary` consumes completed
`controlled-loop-runner-stage-retry-approval.v1` evidence, the planned retry
packet, and the same source evidence chain. Continuation retry boundaries also
require the matching continuation and stage-input binding packets plus the
reviewed stage-input binding checksum. The command emits a reviewed retry
invocation boundary for retry attempt `1` only: exact argv, normalized
arguments, fixed cwd, retry output file, timeout, execution authority, allowed
side-effect policy, and `stage_retry_boundary_checksum`. It does not start a
runner, start a process, execute the retry, emit stage-execution readiness,
select another stage, continue the runner or loop, append audit evidence,
invoke an executor, write Git/GitHub state, merge, release, publish packages,
assign roles, or schedule agents.
It re-reads the saved `operator-approval.v1` named by the retry approval,
requires `--expected-operator-id` to match that signed approval, verifies it
with `--approval-secret-env` or `--approval-secret`, and requires the retry
output file to be new, inside the runtime root, and distinct from every input
evidence file and the source stage output file. Continuation
`start-governed-execution` retry boundaries also re-read the source
stage-execution approval, re-verify its embedded executor-task approval, and
recheck the stage-input binding's embedded prior-stage-output and executor-task
anchors before deriving the retry argv approval token.

```bash
agentic-cadence --root examples/first-run/work/runtime controlled-loop-runner-stage-retry-boundary --controlled-loop-runner-stage-retry-approval-file controlled-loop-runner-stage-retry-approval.json --controlled-loop-runner-stage-retry-plan-file controlled-loop-runner-stage-retry-plan.json --controlled-loop-runner-stage-outcome-plan-file controlled-loop-runner-stage-outcome-plan.json --controlled-loop-runner-stage-closeout-file controlled-loop-runner-stage-closeout.json --controlled-loop-runner-stage-execution-file controlled-loop-runner-stage-execution.json --controlled-loop-runner-start-file controlled-loop-runner-start.json --controlled-loop-runner-plan-file controlled-loop-runner-plan.json --controlled-loop-runner-dry-run-file controlled-loop-runner-dry-run.json --stage-cwd /path/to/repo --stage-retry-output-file controlled-loop-runner-stage-retry-output-attempt-1.json --stage-timeout-seconds 300 --retry-attempt 1 --expected-operator-id operator@example.test --approval-secret-env CADENCE_OPERATOR_APPROVAL_SECRET --stage-number 1
```

```bash
agentic-cadence --root examples/first-run/work/runtime controlled-loop-runner-stage-retry-boundary --controlled-loop-runner-stage-retry-approval-file controlled-loop-runner-stage-retry-approval.json --controlled-loop-runner-stage-retry-plan-file controlled-loop-runner-stage-retry-plan.json --controlled-loop-runner-stage-outcome-plan-file controlled-loop-runner-stage-outcome-plan.json --controlled-loop-runner-stage-closeout-file controlled-loop-runner-stage-closeout.json --controlled-loop-runner-stage-execution-file controlled-loop-runner-stage-execution.json --controlled-loop-runner-next-stage-continuation-file controlled-loop-runner-next-stage-continuation.json --controlled-loop-runner-stage-input-binding-file controlled-loop-runner-stage-input-binding.json --expected-stage-input-binding-checksum sha256:<reviewed-stage-input-binding-checksum> --controlled-loop-runner-start-file controlled-loop-runner-start.json --controlled-loop-runner-plan-file controlled-loop-runner-plan.json --controlled-loop-runner-dry-run-file controlled-loop-runner-dry-run.json --stage-cwd /path/to/repo --stage-retry-output-file controlled-loop-runner-continuation-stage-retry-output-attempt-1.json --stage-timeout-seconds 300 --retry-attempt 1 --expected-operator-id operator@example.test --approval-secret-env CADENCE_OPERATOR_APPROVAL_SECRET --stage-number 2
```

Valid retry-boundary packets emit
`controlled-loop-runner-stage-retry-boundary.v1` with
`packet: controlled_loop_runner_stage_retry_boundary`, recheck retry approval,
retry plan, source outcome/closeout/execution, runner start/plan/dry-run, and,
for continuation retries, continuation and input-binding internal anchors. The
...
does not emit a stage-execution readiness target itself; continuation readiness
is prepared later by `controlled-loop-runner-stage-execution-readiness` after
`controlled-loop-runner-stage-input-binding` has bound the concrete stage
inputs. It does not execute the selected stage, retry, continue the loop,
append audit evidence, start a process, invoke an executor, write Git/GitHub
state, merge, release, publish packages, assign roles, or schedule agents.
...
target packet after next-stage selection. It consumes either the saved initial
next-stage packet or a continuation next-stage packet plus matching stage-input
binding packet, then revalidates the runner-start, runner-plan, and dry-run
chain. Valid packets convert the selected stage to
`ready_for_approval_not_executed`, recommend
`review_controlled_runner_stage_execution_readiness`, set
`next_controlled_action: approve_controlled_runner_stage_execution`, emit a
deterministic `stage_execution_approval_target_checksum`, and append no audit
evidence. Continuation-backed packets include
`stage_selection_source: continuation` and bind the continuation and
stage-input binding checksums, including the reviewed expected binding
checksum supplied with `--expected-stage-input-binding-checksum`.
...
readiness, either the initial next-stage packet or the continuation
next-stage plus stage-input binding packets, runner-start, runner-plan,
dry-run, and saved operator-approval packets, revalidates the runner chain and
approval anchors, requires the selected stage to match the requested stage and
approved runner plan, re-verifies the operator-approval signature and expected
operator, and emits
...
boundary, stage-execution approval, readiness, either initial next-stage or
