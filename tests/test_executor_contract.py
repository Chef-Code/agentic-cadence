...
class ExecutorContractTests(unittest.TestCase):
    ...
    def test_execution_run_record_binds_task_result_validation_and_repo(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            self.assertEqual(record["schema_version"], "execution-run.v1")
    ...
    def test_execution_run_record_reports_stable_mismatch_code(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            task_packet = valid_task_packet(root)
            result = valid_result()
            validation = {"packet": "executor_result_validation", "valid": True, "reason": "ok"}
            record = build_execution_run_record(
                run_id="execution-run-1",
                invocation_id="executor-fixture-invocation-1",
                task_file=root / "executor-task.json",
            ...
            record["result_evidence_checksum"] = checksum_json({"tampered": True})
            ...
            self.assertEqual([blocker["code"] for blocker in blockers], ["run_result_checksum_mismatch"])
    ...
    def test_validate_executor_command_rejects_disabled_live_git_actions(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cases = [
                ...
                ("powershell -Command git push origin main", "executor command violates disabled push permission"),
                ("pwsh -Command git push origin main", "executor command violates disabled push permission"),
                ('pwsh -Command "git push origin main"', "executor command violates disabled push permission"),
