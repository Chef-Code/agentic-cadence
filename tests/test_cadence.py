...
class CadenceCliTests(unittest.TestCase):
    ...
    def write_controlled_loop_runner_plan_chain(self, tmp, repo):
        chain = self.write_controlled_loop_run_manifest_approval_chain(tmp, repo)
        approval_result, approval_evidence = self.run_controlled_loop_run_manifest_approval_cli(tmp, chain)
        self.assertEqual(approval_result.returncode, 0, approval_result.stderr)
        approval_evidence_path = Path(tmp) / "controlled-loop-run-manifest-approval.json"
        approval_evidence_path.write_text(json.dumps(approval_evidence), encoding="utf-8")
        chain["controlled_run_manifest_approval_evidence_path"] = approval_evidence_path
    ...
    def write_controlled_loop_runner_dry_run_chain(self, tmp, repo):
        chain = self.write_controlled_loop_runner_execution_approval_chain(tmp, repo)
        approval_result, approval_evidence = self.run_controlled_loop_runner_execution_approval_cli(tmp, chain)
        self.assertEqual(approval_result.returncode, 0, approval_result.stderr)
        approval_evidence_path = Path(tmp) / "controlled-loop-runner-execution-approval.json"
        approval_evidence_path.write_text(json.dumps(approval_evidence), encoding="utf-8")
        chain["controlled_loop_runner_execution_approval_evidence_path"] = approval_evidence_path
        chain["controlled_loop_runner_execution_approval_evidence"] = approval_evidence
        return chain
    ...
    def run_controlled_loop_runner_plan_cli(self, tmp, chain, **overrides):
        values = {
            "controlled_run_manifest_plan_file": chain["controlled_run_manifest_plan_path"],
    ...
    def run_controlled_loop_runner_dry_run_cli(self, tmp, chain, **overrides):
        values = {
            "controlled_loop_runner_plan_file": chain["controlled_loop_runner_plan_path"],
    ...
    def write_controlled_loop_runner_start_approval_chain(self, tmp, repo):
        chain = self.write_controlled_loop_runner_start_readiness_chain(tmp, repo)
        readiness_result, readiness = self.run_controlled_loop_runner_start_readiness_cli(tmp, chain)
        self.assertEqual(readiness_result.returncode, 0, readiness_result.stderr)
        readiness_path = Path(tmp) / "controlled-loop-runner-start-readiness.json"
        readiness_path.write_text(json.dumps(readiness), encoding="utf-8")
        chain["controlled_loop_runner_start_readiness_path"] = readiness_path
        chain["controlled_loop_runner_start_readiness"] = readiness
        approval_path, approval = write_operator_approval(
            Path(tmp) / "operator-approval-controlled-runner-start.json",
            target_checksum=checksum_json(readiness),
    ...
    def run_controlled_loop_runner_start_approval_cli(self, tmp, chain, **overrides):
        values = {
            "controlled_loop_runner_start_readiness_file": chain["controlled_loop_runner_start_readiness_path"],
            "approval_file": chain["controlled_loop_runner_start_approval_path"],
            "approval_secret": OPERATOR_APPROVAL_SECRET,
            "approval_secret_env": None,
        }
        values.update(overrides)
        args = [
            ...
            str(values["controlled_loop_runner_start_readiness_file"]),
            "--approval-file",
            str(values["approval_file"]),
        ]
        if values["approval_secret"] is not None:
    ...
    def write_controlled_loop_runner_start_chain(self, tmp, repo):
        chain = self.write_controlled_loop_runner_start_approval_chain(tmp, repo)
        approval_result, approval_evidence = self.run_controlled_loop_runner_start_approval_cli(tmp, chain)
        self.assertEqual(approval_result.returncode, 0, approval_result.stderr)
        approval_evidence_path = Path(tmp) / "controlled-loop-runner-start-approval.json"
        approval_evidence_path.write_text(json.dumps(approval_evidence), encoding="utf-8")
        chain["controlled_loop_runner_start_approval_evidence_path"] = approval_evidence_path
        chain["controlled_loop_runner_start_approval_evidence"] = approval_evidence
        return chain

    def run_controlled_loop_runner_start_cli(self, tmp, chain, **overrides):
        values = {
            "controlled_loop_runner_start_approval_file": chain[
                "controlled_loop_runner_start_approval_evidence_path"
            ],
            "controlled_loop_runner_start_readiness_file": chain["controlled_loop_runner_start_readiness_path"],
            "controlled_loop_runner_dry_run_file": chain["controlled_loop_runner_dry_run_path"],
            "controlled_loop_runner_plan_file": chain["controlled_loop_runner_plan_path"],
        ...
        args = [
            ...
            str(values["controlled_loop_runner_start_approval_file"]),
            "--controlled-loop-runner-start-readiness-file",
            str(values["controlled_loop_runner_start_readiness_file"]),
            "--controlled-loop-runner-dry-run-file",
            str(values["controlled_loop_runner_dry_run_file"]),
    ...
    def run_controlled_loop_runner_next_stage_cli(self, tmp, chain, **overrides):
        values = {
    ...
    def write_controlled_loop_runner_next_stage_chain(self, tmp, repo):
        ...
        start_path.write_text(json.dumps(start_output), encoding="utf-8")
        chain["controlled_loop_runner_start_path"] = start_path
        chain["controlled_loop_runner_start"] = start_output
        next_stage_result, next_stage_output = self.run_controlled_loop_runner_next_stage_cli(tmp, chain)
        self.assertEqual(next_stage_result.returncode, 0, next_stage_result.stderr)
        next_stage_path = Path(tmp) / "controlled-loop-runner-next-stage.json"
        next_stage_path.write_text(json.dumps(next_stage_output), encoding="utf-8")
        chain["controlled_loop_runner_next_stage_path"] = next_stage_path
        chain["controlled_loop_runner_next_stage"] = next_stage_output
        return chain

    def run_controlled_loop_runner_stage_execution_readiness_cli(self, tmp, chain, **overrides):
        values = {
    ...
    def test_controlled_loop_runner_plan_accepts_approved_manifest_without_side_effects(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_plan_chain(tmp, repo)
            audit_before = audit_records(tmp)
            files_before = {
                "manifest": chain["controlled_run_manifest_plan_path"].read_text(encoding="utf-8"),
                "approval": chain["controlled_run_manifest_approval_evidence_path"].read_text(encoding="utf-8"),
                "operator_approval": chain["controlled_run_manifest_approval_path"].read_text(encoding="utf-8"),
            }

            result, output = self.run_controlled_loop_runner_plan_cli(tmp, chain)

            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(output["schema_version"], "controlled-loop-runner-plan.v1")
            self.assertEqual(output["packet"], "controlled_loop_runner_plan")
            self.assertTrue(output["read_only"])
            self.assertTrue(output["valid"])
            self.assertEqual(output["runner_plan_status"], "completed")
            self.assertEqual(output["recommended_next_action"], "review_controlled_runner_plan")
            self.assertEqual(output["next_controlled_action"], "request_controlled_runner_execution_approval")
            self.assertTrue(output["operator_confirmation_required"])
            for flag in [
                "runner_started",
                "executor_started",
            ...
            self.assertNotIn("audit_record", output)
            self.assertEqual(output["blockers"], [])
            self.assertEqual(output["approved_manifest"]["checksum"], checksum_json(chain["controlled_run_manifest_plan"]))
            self.assertEqual(
                output["manifest_approval"]["checksum"],
                checksum_json(chain["controlled_run_manifest_approval_evidence"]),
            )
            self.assertEqual(output["runner_plan"]["mode"], "dry_run")
            self.assertEqual(output["runner_plan"]["manifest_checksum"], checksum_json(chain["controlled_run_manifest_plan"]))
            self.assertEqual(output["runner_plan"]["approval_checksum"], checksum_json(chain["controlled_run_manifest_approval_evidence"]))
            self.assertEqual(
                [step["command"] for step in output["runner_plan"]["planned_command_sequence"]],
                [
                    "loop-run-plan",
                    "start-governed-execution",
                    "controlled-loop-start",
            ...
            self.assertEqual(
                {step["status"] for step in output["runner_plan"]["planned_steps"]},
                {"not_started"},
            )
            self.assertEqual(audit_records(tmp), audit_before)
            self.assertEqual(
    ...
    def test_controlled_loop_runner_plan_blocks_manifest_drift_after_approval(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_plan_chain(tmp, repo)
            manifest = dict(chain["controlled_run_manifest_plan"])
            ...
            self.assertEqual(output["recommended_next_action"], "refresh_controlled_run_manifest_approval")
            self.assertIn(
    ...
    def test_controlled_loop_runner_plan_blocks_unaccepted_manifest_approval(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_plan_chain(tmp, repo)
            ...
            approval["recommended_next_action"] = "fix_controlled_run_manifest_approval"
    ...
    def test_controlled_loop_runner_dry_run_accepts_approved_execution_without_side_effects(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_dry_run_chain(tmp, repo)
            audit_before = audit_records(tmp)
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(output["schema_version"], "controlled-loop-runner-dry-run.v1")
            self.assertEqual(output["packet"], "controlled_loop_runner_dry_run")
            self.assertTrue(output["read_only"])
            self.assertTrue(output["valid"])
            self.assertEqual(output["runner_dry_run_status"], "completed")
            self.assertEqual(output["recommended_next_action"], "review_controlled_runner_dry_run")
            self.assertEqual(output["next_controlled_action"], "stop_after_controlled_runner_dry_run")
            self.assertTrue(output["operator_confirmation_required"])
            for flag in [
                "runner_started",
                "executor_started",
            ...
            self.assertTrue(output["approval"]["signature_verified"])
            self.assertEqual(output["approval"]["blocker_codes"], [])
            self.assertEqual(output["runner_dry_run"]["mode"], "dry_run")
            self.assertEqual(
                [stage["command"] for stage in output["runner_dry_run"]["stages"]],
                [
                    "loop-run-plan",
                    "start-governed-execution",
                    "controlled-loop-start",
            ...
            self.assertEqual({stage["status"] for stage in output["runner_dry_run"]["stages"]}, {"would_process"})
            self.assertEqual(audit_records(tmp), audit_before)
            self.assertEqual(
                {
                    "runner_plan": chain["controlled_loop_runner_plan_path"].read_text(encoding="utf-8"),
    ...
    def test_controlled_loop_runner_dry_run_blocks_mismatched_execution_approval(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_dry_run_chain(tmp, repo)
            approval_evidence = json.loads(json.dumps(chain["controlled_loop_runner_execution_approval_evidence"]))
            approval_details = dict(approval_evidence["approval"])
            approval_details["approval_target_checksum"] = "sha256:" + "0" * 64
            approval_evidence["approval"] = approval_details
    ...
    def test_controlled_loop_runner_dry_run_blocks_malformed_planned_steps(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            ...
            runner_plan_details = dict(runner_plan["runner_plan"])
            runner_plan_details["planned_steps"] = []
            runner_plan["runner_plan"] = runner_plan_details
            chain["controlled_loop_runner_plan_path"].write_text(json.dumps(runner_plan), encoding="utf-8")
            approval_path, approval = write_operator_approval(
                chain["controlled_loop_runner_execution_approval_path"],
                target_checksum=checksum_json(runner_plan),
            ...
            chain["controlled_loop_runner_execution_approval_path"] = approval_path
            approval_evidence = json.loads(json.dumps(chain["controlled_loop_runner_execution_approval_evidence"]))
            approval_evidence["controlled_loop_runner_plan"]["checksum"] = checksum_json(runner_plan)
            approval_evidence["approval"]["checksum"] = checksum_json(approval)
            approval_evidence["approval"]["target_checksum"] = checksum_json(runner_plan)
            approval_evidence["approval"]["approval_target_checksum"] = checksum_json(runner_plan)
            approval_evidence["checksums"]["controlled_loop_runner_plan"] = checksum_json(runner_plan)
            approval_evidence["checksums"]["operator_approval"] = checksum_json(approval)
            chain["controlled_loop_runner_execution_approval_evidence_path"].write_text(
                json.dumps(approval_evidence),
                encoding="utf-8",
            )
    ...
    def test_controlled_loop_runner_dry_run_reports_explicit_non_execution_guarantees(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_dry_run_chain(tmp, repo)
            runtime_before = runtime_tree_manifest(tmp)
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(
                output["non_execution_guarantees"],
                [
            ...
            for stage in output["runner_dry_run"]["stages"]:
    ...
    def test_controlled_loop_runner_start_approval_accepts_ready_packet_without_side_effects(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_start_approval_chain(tmp, repo)
            audit_before = audit_records(tmp)
            ...
            result, output = self.run_controlled_loop_runner_start_approval_cli(tmp, chain)

            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(output["schema_version"], "controlled-loop-runner-start-approval.v1")
            self.assertEqual(output["packet"], "controlled_loop_runner_start_approval")
            self.assertTrue(output["read_only"])
            self.assertTrue(output["valid"])
            self.assertEqual(output["approval_status"], "completed")
            self.assertEqual(output["runner_start_authority"], "operator_approved_not_started")
            self.assertEqual(output["recommended_next_action"], "review_controlled_runner_start_approval")
            self.assertEqual(output["next_controlled_action"], "review_approved_controlled_runner_start")
            self.assertTrue(output["operator_confirmation_required"])
            for flag in [
                "runner_started",
                "executor_started",
            ...
            self.assertEqual(output["blockers"], [])
            self.assertEqual(
                output["controlled_loop_runner_start_readiness"]["checksum"],
    ...
    def test_controlled_loop_runner_start_approval_blocks_stale_readiness_target(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_start_approval_chain(tmp, repo)
            readiness = json.loads(json.dumps(chain["controlled_loop_runner_start_readiness"]))
            readiness["generated_at"] = "2099-01-01T00:00:00Z"
    ...
    def test_controlled_loop_runner_start_approval_revalidates_readiness_packet(self):
        def write_matching_approval(chain, readiness):
            approval_path, approval = write_operator_approval(
        ...
        def blocked_readiness(readiness):
            readiness["valid"] = False
            readiness["runner_start_ready"] = False
        ...
        for name, mutate, expected_code in cases:
            with self.subTest(name=name):
                with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
                    ...
                    readiness = json.loads(json.dumps(chain["controlled_loop_runner_start_readiness"]))
                    mutate(readiness)
                    chain["controlled_loop_runner_start_readiness_path"].write_text(
                        json.dumps(readiness),
                        encoding="utf-8",
                    )
                    write_matching_approval(chain, readiness)
                    audit_before = audit_records(tmp)
                    runtime_before = runtime_tree_manifest(tmp)
    ...
    def test_controlled_loop_runner_start_accepts_approved_start_without_executor_or_continuation(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_start_chain(tmp, repo)
            audit_before = audit_records(tmp)
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(output["schema_version"], "controlled-loop-runner-start.v1")
            self.assertEqual(output["packet"], "controlled_loop_runner_start")
            self.assertTrue(output["valid"])
            self.assertEqual(output["runner_start_status"], "started")
            ...
            self.assertFalse(output["loop_continuation_started"])
            self.assertFalse(output["operator_confirmation_required"])
            self.assertEqual(output["recommended_next_action"], "review_controlled_runner_start")
            self.assertEqual(output["next_controlled_action"], "stop_after_controlled_runner_start")
            self.assertEqual(output["side_effects"], ["controlled_runner_start_audit_appended"])
            self.assertEqual(output["blockers"], [])
            self.assertEqual(
                output["controlled_loop_runner_start_approval"]["checksum"],
                checksum_json(chain["controlled_loop_runner_start_approval_evidence"]),
            )
            self.assertEqual(
                output["controlled_loop_runner_start_readiness"]["checksum"],
                checksum_json(chain["controlled_loop_runner_start_readiness"]),
            ...
            self.assertEqual(
                output["runner_start"]["planned_command_sequence"],
                chain["controlled_loop_runner_start_readiness"]["runner_start_readiness"]["planned_command_sequence"],
            )
            self.assertEqual(len(output["runner_start"]["stages"]), len(output["runner_start"]["planned_command_sequence"]))
            self.assertTrue(all(stage["runner_started"] is True for stage in output["runner_start"]["stages"]))
            self.assertTrue(all(stage["executor_started"] is False for stage in output["runner_start"]["stages"]))
            self.assertEqual(
                output["limitations"],
                [
            ...
            records = audit_records(tmp)
            self.assertEqual(len(records), len(audit_before) + 1)
            self.assertEqual(records[-1]["event"], "controlled_loop_runner_start")
            self.assertEqual(records[-1]["action"], "start_controlled_runner_cycle")
            self.assertEqual(
                records[-1]["controlled_loop_runner_start_approval_checksum"],
                checksum_json(chain["controlled_loop_runner_start_approval_evidence"]),
            )
            self.assertEqual(output["audit_record"]["event"], "controlled_loop_runner_start")
            replay_result, replay_output = run_cli(tmp, "audit-replay")
            self.assertEqual(replay_result.returncode, 0, replay_result.stderr)
            self.assertTrue(replay_output["valid"])

    def test_controlled_loop_runner_next_stage_selects_first_stage_without_execution(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_start_chain(tmp, repo)
            start_result, start_output = self.run_controlled_loop_runner_start_cli(tmp, chain)
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(output["schema_version"], "controlled-loop-runner-next-stage.v1")
            self.assertEqual(output["packet"], "controlled_loop_runner_next_stage")
            self.assertTrue(output["valid"])
            self.assertTrue(output["read_only"])
            self.assertEqual(output["runner_next_stage_status"], "selected")
            self.assertTrue(output["runner_started"])
            self.assertFalse(output["stage_execution_started"])
            self.assertFalse(output["executor_started"])
            self.assertFalse(output["loop_continuation_started"])
            self.assertEqual(output["recommended_next_action"], "review_controlled_runner_next_stage")
            self.assertEqual(output["next_controlled_action"], "prepare_controlled_runner_stage_execution")
            self.assertEqual(output["side_effects"], [])
            self.assertEqual(output["blockers"], [])
            self.assertEqual(output["selected_stage"]["step"], 1)
            self.assertEqual(output["selected_stage"]["command"], "loop-run-plan")
            self.assertEqual(output["selected_stage"]["stage_status"], "selected_not_executed")
            self.assertEqual(output["selected_stage"]["execution_authority"], "read_only")
            self.assertEqual(output["controlled_loop_runner_start"]["checksum"], checksum_json(start_output))
            self.assertEqual(output["controlled_loop_runner_plan"]["checksum"], checksum_json(chain["controlled_loop_runner_plan"]))
            self.assertEqual(
                output["controlled_loop_runner_dry_run"]["checksum"],
                checksum_json(chain["controlled_loop_runner_dry_run"]),
    ...
    def test_controlled_loop_runner_next_stage_does_not_execute_subprocesses(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_start_chain(tmp, repo)
            start_result, start_output = self.run_controlled_loop_runner_start_cli(tmp, chain)
    ...
    def test_controlled_loop_runner_next_stage_blocks_stale_or_malformed_inputs_without_side_effects(self):
        def blocked_start(chain):
            start = json.loads(json.dumps(chain["controlled_loop_runner_start"]))
            start["valid"] = False
            start["runner_start_status"] = "blocked"
            start["runner_started"] = False
            start["blockers"] = [{"code": "example_blocker", "message": "blocked"}]
            chain["controlled_loop_runner_start_path"].write_text(json.dumps(start), encoding="utf-8")

        def stale_runner_plan(chain):
            runner_plan = json.loads(json.dumps(chain["controlled_loop_runner_plan"]))
            runner_plan["runner_plan"]["planned_steps"][0]["command"] = "tampered-command"
            chain["controlled_loop_runner_plan_path"].write_text(json.dumps(runner_plan), encoding="utf-8")

        def stale_dry_run(chain):
            dry_run = json.loads(json.dumps(chain["controlled_loop_runner_dry_run"]))
            dry_run["runner_dry_run"]["stages"] = dry_run["runner_dry_run"]["stages"][:1]
            chain["controlled_loop_runner_dry_run_path"].write_text(json.dumps(dry_run), encoding="utf-8")

        cases = [
            ("blocked-start", blocked_start, "controlled_runner_next_stage_start_not_started"),
            ("stale-runner-plan", stale_runner_plan, "controlled_runner_next_stage_runner_plan_checksum_mismatch"),
            ("stale-dry-run", stale_dry_run, "controlled_runner_next_stage_dry_run_checksum_mismatch"),
        ]
        for name, mutate, expected_code in cases:
            with self.subTest(name=name):
                with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
    ...
    def test_controlled_loop_runner_next_stage_rejects_forged_start_audit_binding(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_start_chain(tmp, repo)
            start_result, start_output = self.run_controlled_loop_runner_start_cli(tmp, chain)
            ...
            tampered_start["reason"] = "tampered after runner-start audit append"
            start_path = Path(tmp) / "controlled-loop-runner-start.json"
    ...
    def test_controlled_loop_runner_next_stage_rejects_malformed_start_audit_summary(self):
        def boolean_chain_index(start):
            start["audit_record"]["chain_index"] = True

        def forged_summary_payload_checksum(start):
            start["audit_record"]["payload_checksum"] = "sha256:" + "0" * 64

        cases = [
            ("boolean-chain-index", boolean_chain_index),
            ("forged-summary-payload-checksum", forged_summary_payload_checksum),
        ]
    ...
    def test_controlled_loop_runner_next_stage_rejects_malformed_start_audit_chain_line(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_start_chain(tmp, repo)
            start_result, start_output = self.run_controlled_loop_runner_start_cli(tmp, chain)
            self.assertEqual(start_result.returncode, 0, start_result.stderr)
            records = audit_records(tmp)
            records[-1]["audit_chain_version"] = "cadence-audit-chain.v0"
            ...
            tampered_start = json.loads(json.dumps(start_output))
            tampered_start["audit_record"]["audit_chain_version"] = records[-1]["audit_chain_version"]
            tampered_start["audit_record"]["event_hash"] = records[-1]["event_hash"]
            start_path = Path(tmp) / "controlled-loop-runner-start.json"
            start_path.write_text(json.dumps(tampered_start), encoding="utf-8")
            chain["controlled_loop_runner_start_path"] = start_path
            chain["controlled_loop_runner_start"] = tampered_start
    ...
    def test_controlled_loop_runner_next_stage_revalidates_dry_run_boundaries(self):
        def write_chain_with_mutated_dry_run(chain, tmp, dry_run):
            dry_run_path = chain["controlled_loop_runner_dry_run_path"]
            dry_run_path.write_text(json.dumps(dry_run), encoding="utf-8")
            dry_run_checksum = checksum_json(dry_run)
            start = json.loads(json.dumps(chain["controlled_loop_runner_start"]))
            start["controlled_loop_runner_dry_run"]["checksum"] = dry_run_checksum
            start["checksums"]["controlled_loop_runner_dry_run"] = dry_run_checksum
            start["runner_start"]["dry_run_checksum"] = dry_run_checksum
            start_path = Path(tmp) / "controlled-loop-runner-start.json"
            start_path.write_text(json.dumps(start), encoding="utf-8")
            chain["controlled_loop_runner_start_path"] = start_path
            chain["controlled_loop_runner_start"] = start

        def started_executor(dry_run):
            dry_run["executor_started"] = True

        def wrote_side_effects(dry_run):
            dry_run["side_effects"] = ["executor_started"]

        def missing_guarantees(dry_run):
            dry_run["non_execution_guarantees"] = []
        ...
        def stale_plan_anchor(dry_run):
            dry_run["runner_dry_run"]["runner_plan_checksum"] = "sha256:" + "0" * 64

        def stale_stage_status(dry_run):
            dry_run["runner_dry_run"]["stages"][0]["status"] = "started"

        cases = [
            ("started-executor", started_executor, "controlled_runner_next_stage_dry_run_authority_flags_invalid"),
            ("wrote-side-effects", wrote_side_effects, "controlled_runner_next_stage_dry_run_not_completed"),
            (
                "missing-guarantees",
                missing_guarantees,
            ...
            (
                "stale-plan-file-anchor",
                stale_plan_file_anchor,
                "controlled_runner_next_stage_dry_run_runner_plan_file_mismatch",
            ),
            ("stale-plan-anchor", stale_plan_anchor, "controlled_runner_next_stage_dry_run_runner_plan_checksum_mismatch"),
            (
                "stale-stage-status",
                stale_stage_status,
        ...
        for name, mutate, expected_code in cases:
            with self.subTest(name=name):
                with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
                    ...
                    dry_run = json.loads(json.dumps(chain["controlled_loop_runner_dry_run"]))
                    mutate(dry_run)
                    write_chain_with_mutated_dry_run(chain, tmp, dry_run)
                    audit_before = audit_records(tmp)
                    runtime_before = runtime_tree_manifest(tmp)
    ...
    def test_controlled_loop_runner_next_stage_only_supports_first_stage_for_task_53(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            ...
            result, output = self.run_controlled_loop_runner_next_stage_cli(tmp, chain, stage_number=2)

            self.assertEqual(result.returncode, 2, result.stderr)
            self.assertFalse(output["valid"])
            self.assertEqual(output["runner_next_stage_status"], "blocked")
            self.assertIn("controlled_runner_next_stage_unsupported_stage", {blocker["code"] for blocker in output["blockers"]})
            self.assertFalse(output["stage_execution_started"])
            self.assertFalse(output["executor_started"])
            self.assertEqual(output["side_effects"], [])
            self.assertEqual(audit_records(tmp), audit_before)
    ...
    def test_controlled_loop_runner_stage_execution_readiness_prepares_stage_without_execution(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            ...
            result, output = self.run_controlled_loop_runner_stage_execution_readiness_cli(tmp, chain)

            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(output["schema_version"], "controlled-loop-runner-stage-execution-readiness.v1")
            self.assertEqual(output["packet"], "controlled_loop_runner_stage_execution_readiness")
            self.assertTrue(output["valid"])
            self.assertTrue(output["read_only"])
            self.assertEqual(output["runner_stage_execution_readiness_status"], "ready")
            self.assertTrue(output["runner_started"])
            self.assertFalse(output["stage_execution_started"])
            self.assertFalse(output["executor_started"])
            self.assertFalse(output["loop_continuation_started"])
            self.assertEqual(output["recommended_next_action"], "review_controlled_runner_stage_execution_readiness")
            self.assertEqual(output["next_controlled_action"], "approve_controlled_runner_stage_execution")
            self.assertEqual(output["side_effects"], [])
            self.assertEqual(output["blockers"], [])
            self.assertEqual(output["selected_stage"]["step"], 1)
            self.assertEqual(output["selected_stage"]["command"], "loop-run-plan")
            self.assertEqual(output["selected_stage"]["source_stage_status"], "selected_not_executed")
            self.assertEqual(output["selected_stage"]["stage_status"], "ready_for_approval_not_executed")
            self.assertEqual(output["selected_stage"]["execution_authority"], "operator_approval_required")
            self.assertFalse(output["selected_stage"]["stage_execution_started"])
            self.assertFalse(output["selected_stage"]["executor_started"])
            self.assertEqual(output["selected_stage"]["side_effects"], [])
            self.assertEqual(
                output["controlled_loop_runner_next_stage"]["checksum"],
                checksum_json(chain["controlled_loop_runner_next_stage"]),
            )
            self.assertEqual(
                output["controlled_loop_runner_start"]["checksum"],
                checksum_json(chain["controlled_loop_runner_start"]),
            )
            self.assertEqual(
                output["controlled_loop_runner_plan"]["checksum"],
                checksum_json(chain["controlled_loop_runner_plan"]),
            ...
            approval_target = output["stage_execution_approval_target"]
            self.assertEqual(approval_target["purpose"], "controlled_loop_runner_stage_execution")
            self.assertEqual(approval_target["stage_number"], 1)
            self.assertEqual(approval_target["command"], "loop-run-plan")
            self.assertEqual(
                approval_target["controlled_loop_runner_next_stage_checksum"],
                checksum_json(chain["controlled_loop_runner_next_stage"]),
            )
            self.assertEqual(
                approval_target["controlled_loop_runner_start_checksum"],
                checksum_json(chain["controlled_loop_runner_start"]),
            )
            self.assertEqual(
                approval_target["controlled_loop_runner_plan_checksum"],
                checksum_json(chain["controlled_loop_runner_plan"]),
            )
            self.assertEqual(
                approval_target["controlled_loop_runner_dry_run_checksum"],
                checksum_json(chain["controlled_loop_runner_dry_run"]),
            )
            self.assertEqual(
                approval_target["selected_stage_checksum"],
                checksum_json(chain["controlled_loop_runner_next_stage"]["selected_stage"]),
            )
            self.assertEqual(
                output["stage_execution_approval_target_checksum"],
                checksum_json(approval_target),
            )
            self.assertIn("does_not_execute_runner_stage", output["limitations"])
            self.assertIn("does_not_invoke_executor", output["limitations"])
    ...
    def test_controlled_loop_runner_stage_execution_readiness_blocks_stale_or_started_inputs(self):
        def started_next_stage(chain):
            next_stage = json.loads(json.dumps(chain["controlled_loop_runner_next_stage"]))
            next_stage["stage_execution_started"] = True
            next_stage["selected_stage"]["stage_status"] = "executed"
            chain["controlled_loop_runner_next_stage_path"].write_text(json.dumps(next_stage), encoding="utf-8")

        def stale_next_stage_plan_checksum(chain):
            next_stage = json.loads(json.dumps(chain["controlled_loop_runner_next_stage"]))
            next_stage["checksums"]["controlled_loop_runner_plan"] = "sha256:" + "0" * 64
            chain["controlled_loop_runner_next_stage_path"].write_text(json.dumps(next_stage), encoding="utf-8")
        ...
        cases = [
            ...
            (
                "stale-next-stage-plan-checksum",
                stale_next_stage_plan_checksum,
        ...
        for name, mutate, expected_code in cases:
            with self.subTest(name=name):
                with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
                    ...
                    self.assertEqual(result.returncode, 2, result.stderr)
                    self.assertFalse(output["valid"])
                    self.assertEqual(output["runner_stage_execution_readiness_status"], "blocked")
                    self.assertFalse(output["stage_execution_started"])
                    self.assertFalse(output["executor_started"])
                    self.assertFalse(output["loop_continuation_started"])
                    self.assertIsNone(output["selected_stage"])
                    self.assertIsNone(output["stage_execution_approval_target"])
                    self.assertIn(expected_code, {blocker["code"] for blocker in output["blockers"]})
                    self.assertEqual(output["side_effects"], [])
                    self.assertEqual(audit_records(tmp), audit_before)
                    self.assertEqual(runtime_tree_manifest(tmp), runtime_before)

    def test_controlled_loop_runner_stage_execution_readiness_only_supports_first_stage_for_task_54(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            init_committed_repo(repo)
            chain = self.write_controlled_loop_runner_next_stage_chain(tmp, repo)
            audit_before = audit_records(tmp)
            ...
            result, output = self.run_controlled_loop_runner_stage_execution_readiness_cli(tmp, chain, stage_number=2)
            ...
            self.assertIsNone(output["selected_stage"])
            self.assertIsNone(output["stage_execution_approval_target"])
            self.assertIn("controlled_runner_stage_execution_readiness_upstream_invalid", {blocker["code"] for blocker in output["blockers"]})
            upstream_codes = {
                blocker.get("upstream_code")
                for blocker in output["blockers"]
                if blocker["code"] == "controlled_runner_stage_execution_readiness_upstream_invalid"
            }
            self.assertIn("controlled_runner_next_stage_unsupported_stage", upstream_codes)
            self.assertEqual(output["side_effects"], [])
            self.assertEqual(audit_records(tmp), audit_before)
            self.assertEqual(runtime_tree_manifest(tmp), runtime_before)
    ...
    def test_controlled_loop_runner_start_blocks_stale_start_approval_chain_without_audit(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as repo:
            ...
            self.assertEqual(result.returncode, 2, result.stderr)
            self.assertFalse(output["valid"])
            self.assertEqual(output["runner_start_status"], "blocked")
            self.assertFalse(output["runner_started"])
            self.assertFalse(output["executor_started"])
            self.assertFalse(output["loop_continuation_started"])
    ...
    def test_controlled_loop_runner_start_revalidates_start_approval_and_saved_chain(self):
        ...
        def blocked_start_approval(chain):
            ...
            approval["valid"] = False
            approval["approval_status"] = "blocked"
            approval["runner_start_authority"] = "none"
            approval["blockers"] = [{"code": "example_blocker", "message": "blocked"}]
            chain["controlled_loop_runner_start_approval_evidence_path"].write_text(
                json.dumps(approval),
                encoding="utf-8",
        ...
        def started_authority(chain):
            approval = json.loads(json.dumps(chain["controlled_loop_runner_start_approval_evidence"]))
            approval["runner_started"] = True
            chain["controlled_loop_runner_start_approval_evidence_path"].write_text(
                json.dumps(approval),
                encoding="utf-8",
            )

        def malformed_readiness_stages(chain):
            readiness = json.loads(json.dumps(chain["controlled_loop_runner_start_readiness"]))
            readiness["runner_start_readiness"]["stages"][0]["dry_run_status"] = "started"
            chain["controlled_loop_runner_start_readiness_path"].write_text(
                json.dumps(readiness),
        ...
        def tampered_dry_run_stage(chain):
            dry_run = json.loads(json.dumps(chain["controlled_loop_runner_dry_run"]))
            dry_run["runner_dry_run"]["stages"] = dry_run["runner_dry_run"]["stages"][:1]
            chain["controlled_loop_runner_dry_run_path"].write_text(json.dumps(dry_run), encoding="utf-8")

        def tampered_runner_plan_steps(chain):
            runner_plan = json.loads(json.dumps(chain["controlled_loop_runner_plan"]))
            runner_plan["runner_plan"]["planned_steps"][0]["command"] = "start-runner-now"
            chain["controlled_loop_runner_plan_path"].write_text(json.dumps(runner_plan), encoding="utf-8")
        ...
        cases = [
            ...
            (
                "malformed-readiness-stages",
                malformed_readiness_stages,
                "controlled_runner_start_readiness_stage_malformed",
            ),
            ("tampered-dry-run-stage", tampered_dry_run_stage, "controlled_runner_start_dry_run_checksum_mismatch"),
            (
                "tampered-runner-plan-steps",
                tampered_runner_plan_steps,
