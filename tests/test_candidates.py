...
""",
    ...
    def assert_repo_business_memory_seeds_resume_continuation_backlog_without_warnings(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(business_memory_candidates), 1)
            candidate = business_memory_candidates[0]
            self.assertEqual(candidate["evidence"]["heading"], "Resume Verification Needs An Execution Continuation Gate")
            self.assertEqual(candidate["classification"], "risk")
            self.assertEqual(candidate["classification_confidence"], "high")
            self.assertEqual(candidate["workflow"], "Context-window shutdown and pickup")

    def test_repo_business_memory_current_entries_are_closed_and_parse_without_warnings(self):
        self.assert_repo_business_memory_seeds_resume_continuation_backlog_without_warnings()

    def test_repo_business_memory_current_entries_seed_resume_continuation_backlog_and_parse_without_warnings(self):
        self.assert_repo_business_memory_seeds_resume_continuation_backlog_without_warnings()
